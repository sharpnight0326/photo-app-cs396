...
const posts2HTML = post => {
    ...
    <div class="post">
        ...
        <div class="caption">
            <p><strong>${post.user.username}</strong> ${post.caption?post.caption:'no caption'}</p>
    ...
    fetch("/api/comments", {
        ...
        .then(data => {
            fetch(`/api/posts/${postId}`)
                .then(response => response.json())
                .then(data => {
                    const commentClassName = `viewcomments ${postId}`
                    document.querySelector("[class=" + CSS.escape(commentClassName) + "]").innerHTML = `View all ${data.comments.length} comments`
                });
        })
    // console.log(comment)
}
